...
var removeIcon = Exponent.ICON_RELATIVE+"delete.gif";
var removeDisabledIcon = Exponent.ICON_RELATIVE+"delete.disabled.gif";
var lastTd = null;
var imageSuffix = ".gif";
...
function regenerateTable() {
	...
	var tbody = document.getElementById("ToolbarDesigner_workspace");
	recurseClear(tbody);
		...
		for (key2 in used) {
			if (used[key2] == key) {
				...
				if (document.all) {
					...
					td.setAttribute("class", 'ToolbarDesigner_toolboxbutton');
...
function disableToolbox(icon) {
	if (icon != "space" && icon != "separator") {
		...
		if (document.all) {
			...
			td.setAttribute("class", 'ToolbarDesigner_toolboxbutton_selected');
...
function exponentJSbuildHTMLEditorButtonSelector(Buttons) {
	myButtonPanel = document.getElementById("ToolbarDesigner_toolbox");
	...
	for (currButton in Buttons) {
		...
		myButton_td.setAttribute('class' , 'ToolbarDesigner_toolboxbutton');
