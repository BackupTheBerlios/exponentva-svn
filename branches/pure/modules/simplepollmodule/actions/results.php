...
if ($question) {
	if ($question->open_results == 1) {
		...
		if (!function_exists('pathos_sorting_byVoteCountDescending')) {
			function pathos_sorting_byVoteCountDescending($a,$b) {
				return ($a->vote_count > $b->vote_count ? -1 : 1);
			}
		}
		uasort($answers,'pathos_sorting_byVoteCountDescending');
