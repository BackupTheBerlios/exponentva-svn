...
<div class="weblog_Mini_Calendar" id='jscalendar{$__loc->src|replace:"@":"_"}'>
	...
	{literal}	
		...
		function dateChanged{/literal}{$__loc->src|replace:"@":"_"}{literal}(calendar) {
			...
			if (calendar.dateClicked) {
				...
				window.location.href = "{/literal}{$smarty.const.SCRIPT_RELATIVE}{$smarty.const.SCRIPT_FILENAME}?time=" + nDate + "&action=view_day&module={$__loc->mod}&src={$__loc->src}"{literal};
			};
		};
...
<div class="weblog_Mini_Calendar">
	<a class="mngmntlink weblog_mngmntlink" href="{link action=view_month}" title="{#i18n_viewmonthof#} {$now|format_date:"%B"}" alt="{#i18n_viewmonthof#} {$now|format_date:"%B"}">{#i18n_viewmonth#}</a>
	<br />
	{permissions level=$smarty.const.UILEVEL_NORMAL}
		{if $permissions.post == 1}
			<a class="mngmntlink weblog_mngmntlink" href="{link action=edit_post}" title="{#i18n_create_desc#}" alt="{#i18n_create_desc#}">{#i18n_create#}</a><br />
		{/if}
		{if $in_approval != 0 && $canview_approval_link == 1}
			<a class="mngmntlink weblog_mngmntlink" href="{link module=workflow datatype=weblog m=weblogmodule s=$__loc->src action=summary}" title="{#i18n_viewapprovals_desc#}" alt="{#i18n_viewapprovals_desc#}">{#i18n_viewapprovals#}</a><br />
		{/if}
	{/permissions}
	<br />
	
	{if $modconfig->enable_categories == 1}
		<a href="{link module=categories m=weblogmodule action=manage}" class="mngmntlink weblog_mngmntlink">{#i18n_managecategories#}</a>
	{/if}
</div>