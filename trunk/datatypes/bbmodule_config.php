...
class bbmodule_config {
	function form($object) {
		...
		$form->register(null,'',new htmlcontrol('<hr size="1" />"New Thread" Email'));
		$form->register('email_title_thread','Message Title',new textcontrol($object->email_title_thread));
		$form->register('email_from_thread','From (Display)',new textcontrol($object->email_from_thread));
		$form->register('email_address_thread','From (Email)',new textcontrol($object->email_address_thread));
		$form->register('email_reply_thread','Reply-to',new textcontrol($object->email_reply_thread));
		$form->register('email_showpost_thread','Show post in message?',new checkboxcontrol($object->email_showpost_thread,true));
		
		$form->register(null,'',new htmlcontrol('<hr size="1" />"New Reply" Email'));
		$form->register('email_title_reply','Message Title',new textcontrol($object->email_title_reply));
		$form->register('email_from_reply','From (Display)',new textcontrol($object->email_from_reply));
		$form->register('email_address_reply','From (Email)',new textcontrol($object->email_address_reply));
		$form->register('email_reply_reply','Reply-to',new textcontrol($object->email_reply_reply));
		$form->register('email_showpost_reply','Show post in message?',new checkboxcontrol($object->email_showpost_reply,true));
		
		$form->register(null,'',new htmlcontrol('<hr size="1" />General Settings (applies to both email message types)'));
